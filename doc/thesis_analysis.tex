...
\begin{figure}[htbp]
    ...
    \caption{Beta model fits for the combined east-west and north-south regions. The surface brightness profile is show in the top graph and the lower two graphs display the relative surface brightness differences between each region (\(\text{SB}_{\text{sec}}\)) and the full azimuthal surface brightness profile (\(\text{SB}_{\text{an}}\)). Error bars are derived from Gaussian error propagation. Excluding the first data point, both combined surface brightness profiles show no significant deviation from the full azimuthal profile and are entirely consistent with another, indicating no significant east-west elongation in the outskirts of the galaxy group.}
