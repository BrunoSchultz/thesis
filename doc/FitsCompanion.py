...
class PrimCompanion(fits.PrimaryHDU):
    ...
    def contour_plot_galactic(self, norm='lin', levels=10, bin_edges=None, vmin=None, vmax=None, dpi=480, compress=False, grid=True, axis_label=True, cbar=True, regions=True, savefig=False, axislabel_size=10, tick_label_size=9, cbar_labelsize=9, axis_ticks=True, figsize=(5, 3)):
        """
        Create a contour plot of the data in galactic coordinates with customizable bin sizes.
        ...
        wcs_galactic = wcs.deepcopy()
        skycoord = SkyCoord(wcs.wcs.crval[0], wcs.wcs.crval[1], frame='icrs', unit='deg').galactic
        wcs_galactic.wcs.crval = [skycoord.l.deg, skycoord.b.deg]
        wcs_galactic.wcs.ctype = ['GLON-TAN', 'GLAT-TAN']
        ...
        fig = plt.figure(dpi=dpi, tight_layout=True, figsize=figsize)
        ax = fig.add_subplot(111, projection=wcs_galactic)
        ax.set_box_aspect(1)
        ...
        if axis_label:
            ra.set_axislabel('Galactic Longitude', size=axislabel_size, minpad=.7)
            dec.set_axislabel('Galactic Latitude', size=axislabel_size, minpad=.7)
