...
class PrimCompanion(fits.PrimaryHDU):
    ...
    def fgauss(self, sigma, inplace=False):
        ...
        Returns:
        - PrimCompanion: A new PrimCompanion object with the filtered data if inplace is False, None otherwise.
        """
    ...
    def plot(self, norm='lin', vmin=0, vmax=1, dpi=300, compress=False, grid=True, label=True, cbar=True, regions=True):
        """
        ...
        if label:
            ...
            ra.tick_params(direction='in', color='white')
            dec.tick_params(direction='in', color='white')
            ...
            ra.set_axislabel('Right ascension', size=10, minpad=.7)
            dec.set_axislabel('Declination', size=10, minpad=.7)
        ...
        if cbar:
            ...
            fig_cbar.ax.tick_params(labelsize=7) 
